...
export default function DetailedResultsPage() {
  ...
  return (
    <Grid container className="DetailedResultsPage layout" justifyContent="center" sx={styles.grid}>
      <Grid item xs={12} key="detailedResultsHeader">
        <Header text={`Detailed results of ${contestId}`} />
      </Grid>
      <Grid item xs={12} md={6} key="detailedResultsGrid" sx={{ ...styles.items, textAlign: 'center' }}>
        <DetailedResultsGrid results={contest.results} />
        <Button
          onClick={() => xlsService.exportResultsXls(contest.results, contest.id)}
          sx={styles.button}
        >
          Download
        </Button>
