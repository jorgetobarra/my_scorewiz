...
export function getParticipant(contestId: string, participantName: string): Participant | undefined {
  const contest = getContest(contestId);
  return contest?.participants?.find((p) => p.name === participantName);
}

export function setParticipants(contestId: string, participants: Participant[]): void {
  const contest = getContest(contestId);
  contest.participants = participants; // TODO: this overrides votes, careful
  setContest(contest);
}

export function addParticipant(contestId: string, participant: Participant): void {
  const contest = getContest(contestId);
...
export function setParticipant(contest: Contest, participant: Participant): void { // TODO: what does this do?
...
export function removeParticipant(contestId: string, participantId: string): void {
  const contest = getContest(contestId);
...
export function setAllVotes(contestId: string): Participant[] | undefined {
  ...
  contest.participants?.sort((a, b) => (b.points || 0) - (a.points || 0)) // Descending order
...
export function getVotes(contestId: string, participantId: string): Vote[] | undefined {
  const contest = getContest(contestId);
  return contest?.participants?.find((p) => p.id === participantId)?.votes;
}

export function setVotes(contestId: string, participantId: string, votes: Vote[] | undefined): void {
  const contest = getContest(contestId);
