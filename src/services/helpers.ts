...
export const mapResultsToGrid = (contest: Contest): any[][] => {
  ...
  const sortedResults = participants.sort(
    (a, b) => (a.place || 0) - (b.place || 0)
  );
  sortedResults.forEach((p, indexP) => {
    ...
    sortedResults.forEach((r, indexR) => {
      resultsGrid[0][indexR + 2] = r.name;
      resultsGrid[indexP + 1][indexR + 2] =
        r.votes?.find((v) => v.participantId === p.id)?.points || 0;
    });
